...
$(document).ready(function () {
    ...
    if ($(window).innerWidth() >= 768) {
        $(window).scroll(function () { // load image xettuyen
            if ($(window).scrollTop() >= 1000) {
                $('.back_to_diem').addClass('act');
            }
            else {
                $('.back_to_diem').removeClass('act');
            }
        });
    } else {
        $('.back_to_diem').removeClass('act');
    ...
    $(document).ready(function () {
        ...
        function processData(data) {  // Khi sccess = true
            ...
            for (var j = 1; j < lines.length; j++) {
                ...
                else if ((HoTen != txtName) && (Ngaysinh != txtDate)) {
            ...
            for (var j = 1; j < lines.length; j++) {
                ...
                if ((HoTen == txtName) && (Ngaysinh == txtDate)) { // Truong hop Dung
                    setTimeout(function () {
                        ...
                        document.getElementById('text_nganh').innerHTML = "Đã đạt điểm trúng tuyển bậc Đại học, hệ chính quy, ngành:" + "<p class='font-weight-bold pd_left'> " + Nganh_xettuyen + "</p>";
...
if ((month < 5) || (month > 12)) {  // thang < 5 hoac > 12 thi layout tra cuu khong hien thi
    ...
    document.getElementById("back_to_diem").style.display = "none";
}
// End
...
dropdownYear = document.getElementById("txtNgaySinhYear");
currentYear = (new Date()).getFullYear();
startYear = currentYear - 22;
